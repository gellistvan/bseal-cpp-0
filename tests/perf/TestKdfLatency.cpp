...
namespace {
    ...
    TEST(KdfLatency, FastPresetCompletesWithinBudget) {
        ...
        EXPECT_GE(result.size(), kMinOutputBytes) << "KDF output shorter than expected";
    ...
    TEST(KdfLatency, DifferentSaltsProduceDifferentOutputs) {
        ...
        const bool equal = std::equal(out_a.data(), out_a.data() + out_a.size(), out_b.data());
    ...
    TEST(KdfLatency, SameInputsProduceSameOutput) {
        ...
        const bool equal = std::equal(out1.data(), out1.data() + out1.size(), out2.data());
        EXPECT_TRUE(equal)
            << "KDF is not deterministic — encrypt/decrypt key mismatch would follow";
    }
