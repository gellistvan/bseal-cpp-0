...
namespace {
    ...
    struct AeadFixture {
        ...
        explicit AeadFixture(std::size_t key_bytes, std::size_t nonce_bytes)
            : key(make_bytes(key_bytes, 0x11)), nonce(make_bytes(nonce_bytes, 0x22)),
    ...
    void measure_throughput(CryptoBackend &backend, const char *label) {
        ...
        const double enc_secs = std::chrono::duration<double>(enc_end - enc_start).count();
        ...
        const double dec_secs = std::chrono::duration<double>(dec_end - dec_start).count();
        ...
        std::printf("[perf] %s encrypt: %.1f MB/s  decrypt: %.1f MB/s"
                    "  (%.1f MiB in %.3f s / %.3f s)\n",
                    label, enc_mbps, dec_mbps, enc_bytes / (1024.0 * 1024.0), enc_secs, dec_secs);
        std::fflush(stdout);
