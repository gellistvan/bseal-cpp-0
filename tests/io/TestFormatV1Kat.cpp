...
std::array<Byte, 32> kat_archive_id() {
    std::array<Byte, 32> a{};
    for (int i = 0; i < 32; ++i) a[static_cast<std::size_t>(i)] = static_cast<Byte>(i + 1);
    return a;
}

std::array<Byte, 32> kat_kdf_salt() {
    std::array<Byte, 32> a{};
    for (int i = 0; i < 32; ++i) a[static_cast<std::size_t>(i)] = static_cast<Byte>(0x21 + i);
    return a;
}
...
void write_fixture(const char* name, const Bytes& data) {
    ...
    if (!f) throw std::runtime_error("cannot write fixture: " + path.string());
...
void write_fixture(const char* name, const std::array<Byte, N>& data) {
...
TEST(FormatV1Kat, XChaCha20ChunkKeyBytes) {
    ...
    const auto keys = bseal::crypto::expand_keys(
        ConstByteSpan{seed.data(), seed.size()},
...
TEST(FormatV1Kat, XChaCha20NonceChunk0Bytes) {
    ...
    const auto nonce = bseal::crypto::derive_chunk_nonce(
        ConstByteSpan{keys.nonce_derivation_key.data(), keys.nonce_derivation_key.size()},
        ctx, 0u);
...
TEST(FormatV1Kat, ChunkAadBytes) {
    ...
    const bseal::crypto::ChunkAad aad{
...
TEST(FormatV1Kat, RejectBadFormatMajor) {
    ...
    EXPECT_THROW(bseal::io::parse_global_public_header(
                     ConstByteSpan{bad.data(), bad.size()}),
                 bseal::InvalidArgument);
    // Verify good header still parses fine.
    EXPECT_NO_THROW(bseal::io::parse_global_public_header(
        ConstByteSpan{good.data(), good.size()}));
}
...
TEST(FormatV1Kat, RejectNonzeroShardReservedBytes) {
    ...
    EXPECT_THROW(bseal::io::parse_shard_public_header(
                     ConstByteSpan{bad.data(), bad.size()}),
                 bseal::InvalidArgument);
}
...
TEST(FormatV1Kat, RejectNonzeroChunkFrameReserved) {
    ...
    EXPECT_THROW(bseal::io::parse_chunk_frame_header_v1(
                     ConstByteSpan{bad.data(), bad.size()}),
                 bseal::InvalidArgument);
}
