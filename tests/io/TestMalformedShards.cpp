...
bseal::io::ShardWritePosition write_fake_frame(
    ...
    const auto planned = writer.plan_chunk_frame(
        chunk_index, plaintext_len, plaintext_len, kTestTagLen, final_chunk);
...
TEST(MalformedShards, GlobalHeader_TruncatedAtByte100_Throws) {
    ...
    EXPECT_THROW(
        (void)bseal::io::parse_global_public_header(truncated),
...
TEST(MalformedShards, ShardHeader_WrongMagic_Throws) {
    ...
    EXPECT_THROW(
        (void)bseal::io::parse_shard_public_header(
            bseal::ConstByteSpan{bytes.data(), bytes.size()}),
        bseal::InvalidArgument);
}
...
TEST(MalformedShards, ShardHeader_TruncatedAt40_Throws) {
    ...
    EXPECT_THROW(
        (void)bseal::io::parse_shard_public_header(truncated),
        bseal::InvalidArgument);
}
...
TEST(MalformedShards, ShardFile_PayloadLenTooLarge_Throws) {
    ...
    constexpr std::uint64_t kPayloadLenOffset =
        bseal::io::kGlobalPublicHeaderV1Size + 32;
...
TEST(MalformedShards, ShardReader_DuplicateShardIndex_Throws) {
    ...
    bseal::io::ShardWriter writer(
        make_writer_options(dir, frame_size, kTestChunkPlain, 2, 2));
...
TEST(MalformedShards, ShardReader_ReorderedChunkIndex_Throws) {
    ...
    // global_chunk_index at frame offset 12: magic(4)+frame_header_len(2)+frame_flags(2)+shard_index(4)
