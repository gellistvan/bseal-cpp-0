...
namespace {
    ...
    std::vector<std::filesystem::path> list_files_with_extension(const std::filesystem::path &dir,
...
TEST(TestShardWriter, WritesMultipleChunkFramesIntoOneShard) {
    ...
    const auto p0 = write_fake_frame(writer, 0, kTestChunkPlainSize, false,
                                     bseal::ConstByteSpan{c0.data(), c0.size()});

    const auto p1 = write_fake_frame(writer, 1, kTestChunkPlainSize / 2, true,
...
TEST(TestShardWriter, SplitsChunkFramesAcrossMultipleShards) {
    ...
    const std::uint64_t frame_size =
        static_cast<std::uint64_t>(bseal::io::kChunkFrameHeaderV1Size) + kTestChunkPlainSize +
        kTestTagLen;
...
TEST(TestShardWriter, SupportsCustomExtension) {
    ...
    const auto pos = write_fake_frame(writer, 0, kTestChunkPlainSize, true,
                                      bseal::ConstByteSpan{c0.data(), c0.size()});
...
TEST(TestShardWriter, RejectsOutOfOrderFramePlanning) {
    ...
    EXPECT_THROW(
        {
            (void)writer.plan_chunk_frame(1, // skipping chunk 0
                                          kTestChunkPlainSize, kTestChunkPlainSize, kTestTagLen,
                                          true);
        },
...
TEST(TestShardWriter, FinalizationMacsVerifyAgainstFinalGlobalHeader) {
    ...
    for (const auto &path : files) {
        ...
        ASSERT_NO_THROW(global_hdr = bseal::io::parse_global_public_header(
        ...
        ASSERT_NO_THROW(shard_hdr = bseal::io::parse_shard_public_header(
        ...
        EXPECT_EQ(global_hdr.global_chunk_count, 3u) << "wrong global_chunk_count in: " << path;
...
TEST(TestShardWriter, EmptyShardHashVectorThrows) {
    ...
    EXPECT_THROW({ bseal::io::ShardWriter writer(std::move(opts)); }, bseal::InvalidArgument);
...
TEST(TestShardWriter, WrongHashVectorSizeThrows) {
    ...
    EXPECT_THROW({ bseal::io::ShardWriter writer(std::move(opts2)); }, bseal::InvalidArgument);
...
constexpr std::uint64_t kBoundaryFrameSize =
    static_cast<std::uint64_t>(bseal::io::kChunkFrameHeaderV1Size) + kBoundaryChunkSize +
...
TEST(TestShardWriter, PlanChunkFrameRejectsOversizedLimit) {
    ...
    auto opts_small =
        make_writer_options(dir, kBoundaryFrameSize - 1, ".bin", kBoundaryChunkSize, 1, 1);
    bseal::io::ShardWriter writer_small(std::move(opts_small),
                                        bseal::io::UnsafeAllowMissingShardAadForTests{});
    EXPECT_THROW((void)writer_small.plan_chunk_frame(0, kBoundaryChunkSize, kBoundaryChunkSize,
                                                     kTestTagLen, true),
                 bseal::InvalidArgument);
...
TEST(TestShardWriter, PlanChunkFrameAcceptsExactLimit) {
    ...
    auto opts_exact =
        make_writer_options(dir, kBoundaryFrameSize, ".bin", kBoundaryChunkSize, 1, 1);
    bseal::io::ShardWriter writer_exact(std::move(opts_exact),
                                        bseal::io::UnsafeAllowMissingShardAadForTests{});
    EXPECT_NO_THROW((void)writer_exact.plan_chunk_frame(0, kBoundaryChunkSize, kBoundaryChunkSize,
                                                        kTestTagLen, true));
...
TEST(TestShardWriter, WriteChunkFrameRejectsOversizedLimit) {
    ...
    EXPECT_THROW((void)writer_small.write_chunk_frame(
                     hdr, bseal::ConstByteSpan{hdr_bytes.data(), hdr_bytes.size()},
                     bseal::ConstByteSpan{c0.data(), c0.size()}),
                 bseal::InvalidArgument);
...
TEST(TestShardWriter, WriteChunkFrameAcceptsExactLimit) {
    ...
    EXPECT_NO_THROW((void)writer_exact.write_chunk_frame(
        hdr, bseal::ConstByteSpan{hdr_bytes.data(), hdr_bytes.size()},
        bseal::ConstByteSpan{c0.data(), c0.size()}));
...
TEST(TestShardWriter, FinalizationMacFailsWithPlaceholderGlobalHeader) {
    ...
    opts.filename_extension = ".bin";
    opts.header_authentication_key = test_header_authentication_key();
    opts.global_header = make_test_global_header(frame_size, kTestChunkPlainSize, /*shard_count=*/1,
