...
namespace {
    ...
    void create_keyfile(const fs::path &keyfile) {
        write_binary_file(keyfile, std::vector<std::uint8_t>{
                                       0x10,
                                       0x20,
                                       0x30,
                                       0x40,
    ...
    std::string make_command_line(const std::vector<std::string> &args, const fs::path &stdin_file,
    ...
    ProcessResult run_bseal(const fs::path &scratch_dir, const std::vector<std::string> &args,
        ...
        const int raw_rc =
            std::system(make_command_line(args, stdin_file, stdout_file, stderr_file).c_str());
    ...
    std::vector<std::string> encrypt_args(const fs::path &input, const fs::path &output,
        ...
        return {
            ...
            "fast",
            "--chunk-size",
            "64K", // minimum valid per FORMAT.md §3 (65536 bytes)
            "--shard-size",
            "65592", // exactly one frame per shard (40 header + 65536 data + 16
                     // tag)
            "--padding",
            "none",
        };
    ...
    std::vector<std::string> decrypt_args(const fs::path &input, const fs::path &output,
                                          const fs::path &keyfile) {
        return {
            "decrypt",       "--input",   input.string(),   "--output",
            output.string(), "--keyfile", keyfile.string(),
        };
    }

    std::vector<std::string> encrypt_args_with_keyfiles(const fs::path &input,
    ...
    std::vector<std::string> decrypt_args_with_keyfiles(const fs::path &input,
                                                        const fs::path &output,
                                                        const std::vector<fs::path> &keyfiles) {
        std::vector<std::string> args{
            "decrypt", "--input", input.string(), "--output", output.string(),
        };
        for (const auto &kf : keyfiles) {
            args.push_back("--keyfile");
            args.push_back(kf.string());
    ...
    void duplicate_first_shard(const fs::path &sealed_dir) {
        ...
        const auto duplicate =
            files.front().parent_path() / ("zz-duplicate-" + files.front().filename().string());
    ...
    FixturePaths create_encrypt_fixture(TempDir &temp) {
        ...
        const auto encrypt_result =
            run_bseal(temp.subdir("encrypt-run"),
                      encrypt_args(paths.input, paths.sealed, paths.keyfile), kPassphrase);
...
TEST(BlackBoxCliRegression, RoundTripPreservesInputTreeByteForByte) {
    ...
    const auto decrypt_result =
        run_bseal(temp.subdir("decrypt-run"),
                  decrypt_args(paths.sealed, paths.output, paths.keyfile), kPassphrase);
...
TEST(BlackBoxCliRegression, WrongPassphraseFailsWithAuthenticationFailure) {
    ...
    const auto decrypt_result = run_bseal(temp.subdir("decrypt-run"),
...
TEST(BlackBoxCliRegression, DuplicateShardFailsOrIsRejectedDeterministically) {
    ...
    const auto first_result =
        run_bseal(temp.subdir("decrypt-run-1"),
                  decrypt_args(paths.sealed, temp.subdir("output-1"), paths.keyfile), kPassphrase);
    const auto second_result =
        run_bseal(temp.subdir("decrypt-run-2"),
                  decrypt_args(paths.sealed, temp.subdir("output-2"), paths.keyfile), kPassphrase);
...
TEST(BlackBoxCliRegression, HardenedExtractOnRoundTripOnPosix) {
    ...
    expect_output_tree_matches_input_tree(paths.input, paths.output);
#else
    EXPECT_NE(result.exit_code, 0) << "--hardened-extract=on should fail on non-POSIX platforms";
#endif
}
...
TEST(BlackBoxCliRegression, HardenedExtractOnFailsOnNonPosix) {
    ...
    const auto result = run_bseal(temp.subdir("decrypt-run"), args, kPassphrase);
    EXPECT_NE(result.exit_code, 0) << "--hardened-extract=on must fail on non-POSIX platforms";
...
TEST(BlackBoxCliRegression, PreexistingBinFileSurvivesFailedEncrypt) {
    ...
    const auto result =
        run_bseal(temp.subdir("encrypt-run"), encrypt_args(input, sealed, keyfile), kPassphrase);
...
TEST(BlackBoxCliRegression, ShardSizeTooSmallForChunkFails) {
    ...
    const auto result =
        run_bseal(temp.subdir("run"),
                  {"encrypt", "--input", input.string(), "--output", sealed.string(), "--keyfile",
                   keyfile.string(), "--suite", "xchacha20-poly1305", "--kdf", "fast",
                   "--chunk-size", "64K", "--shard-size", "65591", "--padding", "none"},
                  kPassphrase);
...
TEST(BlackBoxCliRegression, ShardSizeExactlyOneFrameSucceeds) {
    ...
    const auto enc =
        run_bseal(temp.subdir("enc-run"),
                  {"encrypt", "--input", input.string(), "--output", sealed.string(), "--keyfile",
                   keyfile.string(), "--suite", "xchacha20-poly1305", "--kdf", "fast",
                   "--chunk-size", "64K", "--shard-size", "65592", "--padding", "none"},
                  kPassphrase);

    EXPECT_EQ(enc.exit_code, 0) << enc.stderr_text;

    const auto dec =
        run_bseal(temp.subdir("dec-run"), decrypt_args(sealed, output, keyfile), kPassphrase);
...
namespace {
    ...
    void patch_file_u16_le(const fs::path &file, std::uint64_t offset, std::uint16_t value) {
        ...
        if (!f)
            throw std::runtime_error("patch_file_u16_le: cannot open " + file.string());
    ...
    void patch_file_u64_le(const fs::path &file, std::uint64_t offset, std::uint64_t value) {
        std::fstream f(file, std::ios::binary | std::ios::in | std::ios::out);
        if (!f)
            throw std::runtime_error("patch_file_u64_le: cannot open " + file.string());
        f.seekp(static_cast<std::streamoff>(offset), std::ios::beg);
    ...
    void append_to_file(const fs::path &file, std::size_t count, std::uint8_t fill = 0xDEu) {
        ...
        if (!f)
            throw std::runtime_error("append_to_file: cannot open " + file.string());
...
TEST(BlackBoxCliRegression, TruncatedGlobalHeader_Fails) {
    ...
    ASSERT_EQ(
        run_bseal(temp.subdir("enc"), encrypt_args(input, sealed, keyfile), kPassphrase).exit_code,
        0);
    ...
    const auto result =
        run_bseal(temp.subdir("dec"), decrypt_args(sealed, output, keyfile), kPassphrase);
    EXPECT_EQ(result.exit_code, 1) << result.stderr_text;
}
...
TEST(BlackBoxCliRegression, WrongKeyfileReturnsExitCode3) {
    ...
    ASSERT_EQ(run_bseal(temp.subdir("enc"), encrypt_args(input, sealed, keyfile_a), kPassphrase)
                  .exit_code,
              0);

    const auto result =
        run_bseal(temp.subdir("dec"), decrypt_args(sealed, output, keyfile_b), kPassphrase);

    EXPECT_EQ(result.exit_code, 3) << result.stderr_text;
    EXPECT_TRUE(contains_text(to_lower_ascii(result.stderr_text), "auth")) << result.stderr_text;
...
TEST(BlackBoxCliRegression, ReorderedKeyfilesReturnsExitCode3) {
    ...
    const auto result =
        run_bseal(temp.subdir("dec"),
                  decrypt_args_with_keyfiles(sealed, output, {keyfile_b, keyfile_a}), kPassphrase);
...
TEST(BlackBoxCliRegression, TamperedShardHeaderMacReturnsExitCode3) {
    ...
    EXPECT_EQ(result.exit_code, 3) << result.stderr_text;
    EXPECT_TRUE(contains_text(to_lower_ascii(result.stderr_text), "auth")) << result.stderr_text;
    EXPECT_FALSE(contains_text(to_lower_ascii(result.stderr_text), " mac")) << result.stderr_text;
...
TEST(BlackBoxCliRegression, KdfPolicyViolationReturnsExitCode1) {
    ...
    const auto result =
        run_bseal(temp.subdir("dec"),
                  {"decrypt", "--input", sealed.string(), "--output", output.string(), "--keyfile",
                   keyfile.string(), "--max-kdf-memory", "64M"},
                  kPassphrase);

    EXPECT_EQ(result.exit_code, 1) << result.stderr_text;
    EXPECT_NE(result.exit_code, 3) << "KDF policy violation must be exit 1, not auth failure";
}
