...
std::string shell_quote_arg(std::string_view a) {
    std::string out = "\"";
    for (char c : a) { if (c == '"') out += "\\\""; else out += c; }
    out += "\""; return out;
}
...
std::string shell_quote(const fs::path& p) {
    std::string out = "'";
    for (char c : p.string()) { if (c == '\'') out += "'\\''"; else out += c; }
    out += "'"; return out;
}
std::string shell_quote_arg(std::string_view a) {
    std::string out = "'";
...
struct BsealResult { int rc; std::string err; };

BsealResult run_bseal(const fs::path& scratch,
    ...
    for (const auto& a : args) { cmd += " "; cmd += shell_quote_arg(a); }
...
TEST(FormatV1ArchiveFixture, EncryptDecryptRoundTrip) {
    ...
    if (regen_mode()) {
        ...
        const auto enc_res = run_bseal(enc_scratch,
            {"encrypt",
             "--input",  input_dir.string(),
             "--output", sealed_dir.string(),
             "--suite",  "xchacha20-poly1305",
             "--kdf",    "fast",
             "--chunk-size", "64K",
             "--padding", "none"},
    ...
    for (const auto& shard : shards) {
        fs::copy_file(shard, sealed_dir / shard.filename(),
                      fs::copy_options::overwrite_existing);
    }
    ...
    const auto dec_res = run_bseal(dec_scratch,
        {"decrypt",
         "--input",  sealed_dir.string(),
    ...
    EXPECT_EQ(actual, expected)
        << "decrypted content differs from fixture expected_content.txt";
}
...
TEST(FormatV1ArchiveFixture, WrongPassphraseFails) {
    ...
    const auto output_dir = scratch.path() / "output";
    const auto dec_res = run_bseal(scratch.path() / "dec_scratch",
        {"decrypt",
         "--input",  sealed_dir.string(),
         "--output", output_dir.string()},
        "wrong-passphrase-bseal-kat");

    EXPECT_EQ(dec_res.rc, 3)
        << "expected exit code 3 for wrong passphrase; got " << dec_res.rc
        << "\nstderr: " << dec_res.err;
}
