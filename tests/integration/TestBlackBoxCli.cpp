...
namespace {
    ...
    void create_sample_input_tree(const fs::path &root) {
        ...
        write_file(root / "nested" / "data.bin", repeated("0123456789abcdef", 512));
        ...
        write_binary_file(root / "nested" / "binary.dat",
                          std::vector<std::uint8_t>{0x00, 0x01, 0x02, 0x7F, 0x80, 0xFE, 0xFF, 0x42,
                                                    0x53, 0x45, 0x41, 0x4C});
    }
    ...
    std::vector<std::string> encrypt_args(const fs::path &input, const fs::path &output,
                                          const fs::path &keyfile_a, const fs::path &keyfile_b) {
        return {
            "encrypt",
            "--input",
            input.string(),
    ...
    std::vector<std::string> decrypt_args(const fs::path &input, const fs::path &output,
        ...
        std::vector<std::string> args{
            ...
            "--keyfile", keyfile_a.string(), "--keyfile",    keyfile_b.string(),
        };
    ...
    void create_keyfiles(const fs::path &keyfile_a, const fs::path &keyfile_b) {
        write_binary_file(keyfile_a, std::vector<std::uint8_t>{
                                         0x10,
                                         0x20,
                                         0x30,
                                         0x40,
        ...
        write_binary_file(keyfile_b, std::vector<std::uint8_t>{
                                         0xFE,
                                         0xDC,
                                         0xBA,
                                         0x98,
    ...
    void patch_u32_le_at_offset(const fs::path &sealed_dir, std::size_t byte_offset,
    ...
    void write_bytes_at(const fs::path &path, std::size_t offset,
...
TEST(BlackBoxCli, EncryptDecryptRoundTripPreservesFilesAndDirectories) {
    ...
    const auto encrypt_result =
        run_bseal(temp.subdir("encrypt-run"), encrypt_args(input, sealed, key_a, key_b),
                  "integration-passphrase\n");
    ...
    const auto decrypt_result =
        run_bseal(temp.subdir("decrypt-run"), decrypt_args(sealed, output, key_a, key_b),
                  "integration-passphrase\n");
...
TEST(BlackBoxCli, WrongKeyfileDoesNotDecryptArchive) {
    ...
    write_binary_file(wrong_key_b, std::vector<std::uint8_t>{
                                       0x99,
                                       0x88,
                                       0x77,
                                       0x66,
    ...
    const auto decrypt_result =
        run_bseal(temp.subdir("decrypt-run"), decrypt_args(sealed, output, key_a, wrong_key_b),
                  "integration-passphrase\n");
...
TEST(BlackBoxCli, RefusesNonEmptyOutputDirectoryWithoutOverwrite) {
    ...
    const auto decrypt_without_overwrite =
        run_bseal(temp.subdir("decrypt-no-overwrite-run"),
                  decrypt_args(sealed, output, key_a, key_b, false), "integration-passphrase\n");

    EXPECT_NE(decrypt_without_overwrite.exit_code, 0);
    EXPECT_TRUE(fs::exists(output / "preexisting.txt"));

    const auto decrypt_with_overwrite =
        run_bseal(temp.subdir("decrypt-overwrite-run"),
                  decrypt_args(sealed, output, key_a, key_b, true), "integration-passphrase\n");
...
TEST(BlackBoxCli, PassphraseOnlyEncryptDecryptSucceeds) {
    ...
    const auto encrypt_result = run_bseal(temp.subdir("encrypt-run"),
                                          {
                                              "encrypt",
                                              "--input",
                                              input.string(),
...
TEST(BlackBoxCli, TamperedKdfMemoryFailsAuthentication) {
    ...
    const auto result = run_bseal(temp.subdir("decrypt-run"),
                                  {"decrypt", "--input", sealed.string(), "--output",
                                   output.string(), "--keyfile", key.string()},
                                  "passphrase\n");
...
TEST(BlackBoxCli, PaddingNoneSucceeds) {
    ...
    const auto result = run_bseal(temp.subdir("encrypt-run"),
                                  {
                                      "encrypt",
                                      "--input",
                                      input.string(),
...
TEST(BlackBoxCli, DefaultPolicyAcceptsFastPreset) {
    ...
    const auto enc = run_bseal(temp.subdir("encrypt-run"),
                               {
                                   "encrypt",
                                   "--input",
                                   input.string(),
    ...
    const auto dec = run_bseal(temp.subdir("decrypt-run"),
    ...
    const auto result = run_bseal(temp.subdir("run"),
                                  {
                                      "decrypt",
                                      "--input",
                                      temp.subdir("nonexistent").string(),
                                      "--output",
                                      temp.subdir("output").string(),
                                      "--max-kdf-memory",
                                      "0",
                                  },
                                  "passphrase\n");
