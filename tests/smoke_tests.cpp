...
int main() {
    ...
    check(std::any_of(random.begin(), random.end(), [](auto b) { return b != 0; }),
          "random output all zero");

    auto stem = bseal::platform::random_filename_stem();
    check(stem.size() >= 38,
          "filename stem too short"); // 192 bits => ceil(192 / 5) = 39 chars, keep test flexible.
    for (char c : stem) {
        check((c >= 'A' && c <= 'Z') || (c >= '2' && c <= '7'),
              "filename stem uses non-base32 char");
    }

    std::array<unsigned char, 4096> secret{};
    bseal::platform::LockedMemoryRegion lock(secret.data(), secret.size());
    (void)lock.locked();
    check(lock.locked_size() == 0 || lock.locked_size() >= secret.size(),
          "locked region size invalid");

    auto features = bseal::platform::detect_cpu_features();
    (void)features;

    const char *argv[] = {"bseal",
                          "encrypt",
                          "--input",
                          "in",
                          "--output",
                          ...
                          "--padding",
                          "none"};
    auto parsed = bseal::cli::parse_args(static_cast<int>(sizeof(argv) / sizeof(argv[0])),
