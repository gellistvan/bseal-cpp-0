...
inline EntryMetadata file_metadata(std::filesystem::path relative_path,
                                   std::uint64_t size,
...
inline EntryMetadata symlink_metadata(std::filesystem::path relative_path,
                                      std::string target) {
    EntryMetadata metadata;
...
inline bool throws_invalid_argument(Fn&& fn) {
    try {
        fn();
