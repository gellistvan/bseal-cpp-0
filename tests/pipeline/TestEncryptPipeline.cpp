...
namespace bseal::pipeline::test {
    namespace {
        ...
        TEST(EncryptPipeline, ThrowsWhenBackendIsNull) {
            ...
            EncryptPipeline pipeline(make_encrypt_options(), nullptr, make_test_keys(),
        ...
        TEST(EncryptPipeline, ThrowsWhenChunkSizeIsZero) {
            ...
            EncryptPipeline pipeline(options, std::make_unique<TestAeadBackend>(), make_test_keys(),
                                     std::move(archive_writer), std::move(shard_writer));
        ...
        TEST(EncryptPipeline, ThrowsWhenChunkKeySizeDoesNotMatchBackend) {
            ...
            EncryptPipeline pipeline(make_encrypt_options(), std::make_unique<TestAeadBackend>(),
        ...
        TEST(EncryptPipeline, PropagatesWorkerFailureAndDoesNotHang) {
            ...
            write_binary_file(input_root.path() / "large.bin",
