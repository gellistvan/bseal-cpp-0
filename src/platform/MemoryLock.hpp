...
namespace bseal::platform {
    // RAII wrapper for Linux memory hardening. It attempts to page-align the supplied region, mlock
    // it, and exclude it from core dumps via MADV_DONTDUMP. Lock failure is non-throwing and
    // visible through locked(); higher layers can decide whether to fail closed.
