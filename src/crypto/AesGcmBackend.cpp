...
namespace bseal::crypto {
    ...
    Bytes AesGcmBackend::encrypt_chunk(const EncryptChunkRequest &request) {
        ...
        if (EVP_EncryptInit_ex(ctx.get(), nullptr, nullptr, request.key.bytes.data(),
                               request.nonce.bytes.data()) != 1) {
        ...
        if (!aad.empty()) {
            if (EVP_EncryptUpdate(ctx.get(), nullptr, &len, aad.data(),
                                  checked_int_size(aad.size(), "AAD")) != 1) {
        ...
        if (!request.plaintext.empty()) {
            if (EVP_EncryptUpdate(ctx.get(), out.data(), &len, request.plaintext.data(),
                                  checked_int_size(request.plaintext.size(), "plaintext")) != 1) {
    ...
    Bytes AesGcmBackend::decrypt_chunk(const DecryptChunkRequest &request) {
        ...
        if (EVP_DecryptInit_ex(ctx.get(), nullptr, nullptr, request.key.bytes.data(),
                               request.nonce.bytes.data()) != 1) {
            throw Error("AES-256-GCM key/nonce setup failed");
        }
