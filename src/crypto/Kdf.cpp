...
namespace bseal::crypto {
    namespace {
        ...
        void blake3_update_cstr_with_nul(blake3_hasher &hasher, const char *cstr,
                                         std::size_t len_including_nul) {
        ...
        SecureBuffer hkdf_sha256(ConstByteSpan ikm, ConstByteSpan salt, ConstByteSpan info,
            ...
            if (!salt.empty()) {
                if (EVP_PKEY_CTX_set1_hkdf_salt(ctx.get(), salt.data(),
                                                checked_int_size(salt.size(), "HKDF salt")) != 1) {
                    throw Error("HKDF salt setup failed");
                }
            }

            if (EVP_PKEY_CTX_set1_hkdf_key(ctx.get(), ikm.data(),
                                           checked_int_size(ikm.size(), "HKDF IKM")) != 1) {
                throw Error("HKDF key setup failed");
            }

            if (!info.empty()) {
                if (EVP_PKEY_CTX_add1_hkdf_info(ctx.get(), info.data(),
                                                checked_int_size(info.size(), "HKDF info")) != 1) {
        ...
        void require_u32_range(std::uint32_t value, std::uint32_t min_value,
                               std::uint32_t max_value, const char *name) {
    ...
    void validate_kdf_params(const KdfParams &params) {
        require_u32_range(params.memory_kib, kArgon2MemoryKiBMin, kArgon2MemoryKiBMax,
                          "Argon2id memory_kib");

        require_u32_range(params.iterations, kArgon2IterationsMin, kArgon2IterationsMax,
                          "Argon2id iterations");

        require_u32_range(params.parallelism, kArgon2ParallelismMin, kArgon2ParallelismMax,
                          "Argon2id parallelism");

        require_u32_range(params.output_bytes, kArgon2OutputBytesMin, kArgon2OutputBytesMax,
                          "KDF output_bytes");
    }
    ...
    SecureBuffer derive_master_seed(const KdfInput &input) {
        ...
        // FORMAT.md §8: pass_key = Argon2id(password, salt=kdf_salt, memory, iterations,
        // parallelism) The salt is the 32-byte kdf_salt directly; all three cost parameters are
        // honored.
        const int rc = argon2id_hash_raw(
            ...
            input.passphrase_utf8.data(), input.passphrase_utf8.size(),
            input.salt.data(), // 32-byte kdf_salt per FORMAT.md §8
            input.salt.size(), pass_key.data(), pass_key.size());
        ...
        SecureBuffer master = hkdf_sha256(
            ...
            ConstByteSpan{reinterpret_cast<const Byte *>(info.data()), info.size()}, 32);
    ...
    void validate_kdf_resource_policy(const KdfResourcePolicy &policy) {
        ...
        if (policy.max_memory_kib > kArgon2MemoryKiBMax) {
            throw InvalidArgument(
                "KDF resource policy max_memory_kib (" + std::to_string(policy.max_memory_kib) +
                ") exceeds the format maximum (" + std::to_string(kArgon2MemoryKiBMax) + ")");
        }
        ...
        if (policy.max_iterations > kArgon2IterationsMax) {
            throw InvalidArgument(
                "KDF resource policy max_iterations (" + std::to_string(policy.max_iterations) +
                ") exceeds the format maximum (" + std::to_string(kArgon2IterationsMax) + ")");
        }
        ...
        if (policy.max_parallelism > kArgon2ParallelismMax) {
            throw InvalidArgument(
                "KDF resource policy max_parallelism (" + std::to_string(policy.max_parallelism) +
                ") exceeds the format maximum (" + std::to_string(kArgon2ParallelismMax) + ")");
        }
    }
    ...
    void check_kdf_params_against_policy(const KdfParams &params, const KdfResourcePolicy &policy) {
        if (params.memory_kib > policy.max_memory_kib) {
            throw InvalidArgument(
                ...
                std::to_string(policy.max_memory_kib) + " KiB); use --max-kdf-memory to override");
        }
        if (params.iterations > policy.max_iterations) {
            throw InvalidArgument(
                ...
                std::to_string(policy.max_iterations) + "); use --max-kdf-iterations to override");
        }
        if (params.parallelism > policy.max_parallelism) {
            throw InvalidArgument("archive Argon2id parallelism (" +
