...
namespace bseal::crypto {
    ...
    ExpandedKeys expand_keys(ConstByteSpan master_seed, CipherSuite suite) {
...

        {
            ...
            keys.chunk_encryption_key =
                hkdf_sha256(master_seed, ConstByteSpan{}, ConstByteSpan{info.data(), info.size()},
                            key_size_for_suite(suite));
        }

        {
            const Bytes info = make_info("BSEAL manifest key v1", suite);
            keys.manifest_key = hkdf_sha256(master_seed, ConstByteSpan{},
                                            ConstByteSpan{info.data(), info.size()}, 32);
        }

        {
            const Bytes info = make_info("BSEAL header authentication key v1", suite);
            keys.header_authentication_key = hkdf_sha256(
                master_seed, ConstByteSpan{}, ConstByteSpan{info.data(), info.size()}, 32);
        }

        {
            const Bytes info = make_info("BSEAL nonce derivation key v1", suite);
            keys.nonce_derivation_key = hkdf_sha256(master_seed, ConstByteSpan{},
                                                    ConstByteSpan{info.data(), info.size()}, 32);
        }
    ...
    Bytes derive_chunk_nonce(ConstByteSpan nonce_derivation_key, const NonceContext &context,
        ...
        SecureBuffer prefix =
            hkdf_sha256(nonce_derivation_key, ConstByteSpan{salt.data(), salt.size()},
                        ConstByteSpan{info.data(), info.size()}, prefix_len);
