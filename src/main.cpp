...
std::unique_ptr<bseal::crypto::CryptoBackend> make_backend(
    bseal::crypto::CipherSuite suite) {
    switch (suite) {
...
void require_path_exists(const std::filesystem::path& path, std::string_view description) {
    if (!std::filesystem::exists(path)) {
        throw bseal::InvalidArgument(std::string(description) + " does not exist: " + path.string());
    }
}
...
void require_directory(const std::filesystem::path& path, std::string_view description) {
    ...
    if (!std::filesystem::is_directory(path)) {
        throw bseal::InvalidArgument(std::string(description) + " is not a directory: " +
                                     path.string());
    }
}
...
void require_keyfiles_exist(const std::vector<std::filesystem::path>& keyfiles) {
    ...
    for (const auto& keyfile : keyfiles) {
        ...
        if (!std::filesystem::is_regular_file(keyfile)) {
            throw bseal::InvalidArgument("keyfile is not a regular file: " + keyfile.string());
        }
    }
}

template <std::size_t N>
std::array<Byte, N> random_array() {
...
bseal::archive::PublicHeaderV1 make_encrypt_public_header(
    const bseal::cli::EncryptOptions& options,
    const std::array<Byte, 16>& archive_id,
    const std::array<Byte, 32>& kdf_salt,
    const bseal::crypto::KdfParams& kdf_params) {
...
ArchiveOpenContext make_encrypt_context(const bseal::cli::EncryptOptions& options) {
    ...
    auto header = make_encrypt_public_header(
        options,
        context.archive_id,
        context.kdf_salt,
        context.kdf_params);
...
std::vector<std::filesystem::path> find_bin_files(const std::filesystem::path& input_dir) {
    ...
    if (result.empty()) {
        throw bseal::InvalidArgument("input directory does not contain any .bin shard files");
    }
...
bseal::archive::PublicHeaderV1 read_first_public_header(
    ...
    /*
      ...
      This fallback reads a generous prefix from the first .bin and passes it to parse_public_header().
      If your implemented ShardReader exposes a stronger API, prefer that instead.
    */
    ...
    return bseal::archive::parse_public_header(
        ConstByteSpan{prefix.data(), prefix.size()});
}
...
bseal::crypto::ExpandedKeys derive_expanded_keys(
    const ArchiveOpenContext& context,
...
int run_encrypt(const bseal::cli::EncryptOptions& options) {
    ...
    bseal::archive::ArchiveWriter archive_writer(
        bseal::archive::ArchiveWriterOptions{
            options.input,
            options.chunk_size,
            true,
            true,
    ...
    bseal::io::ShardWriter shard_writer(
...
int run_decrypt(const bseal::cli::DecryptOptions& options) {
    require_directory(options.input, "input path");
    require_keyfiles_exist(options.keyfiles);
    ...
    bseal::archive::ArchiveReader archive_reader(
        bseal::archive::ArchiveReaderOptions{
            options.output,
            options.overwrite,
            true,
            true,
    ...
    try {
        ...
        switch (args.command) {
            ...
            case bseal::cli::Command::Decrypt:
                return run_decrypt(args.decrypt);
        }
