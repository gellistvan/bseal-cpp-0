...
namespace bseal::pipeline {
    ...
    namespace {
        ...
        const std::array<Byte, 32> &select_public_header_hash(const EncryptPipelineOptions &options,
        ...
        crypto::ChunkAad make_aad(const EncryptPipelineOptions &options, std::uint32_t shard_index,
        ...
        bool enqueue_planned_chunk(io::ShardWriter &shard_writer,
                                   WorkQueue<PlainChunk> &encrypt_queue, std::uint64_t chunk_index,
                                   Bytes bytes, bool final_chunk, std::uint16_t tag_size) {
            const auto plaintext_len = static_cast<std::uint64_t>(bytes.size());

            auto planned = shard_writer.plan_chunk_frame(
                chunk_index, plaintext_len,
                plaintext_len, // v1: ciphertext_len excludes tag and equals plaintext_len.
                tag_size, final_chunk);
        ...
        bool append_record_bytes_to_chunks(const Bytes &record, std::size_t chunk_size,
                                           Bytes &current_chunk,
            ...
            auto flush_pending_as_non_final = [&]() -> bool {
                ...
                return enqueue_planned_chunk(shard_writer, encrypt_queue, next_chunk_index++,
                                             std::move(bytes), false, tag_size);
            };
            ...
            while (offset < record.size()) {
                ...
                current_chunk.insert(
                    current_chunk.end(), record.begin() + static_cast<std::ptrdiff_t>(offset),
        ...
        void producer_main(archive::ArchiveWriter &archive_writer, io::ShardWriter &shard_writer,
                           const EncryptPipelineOptions &options, std::uint16_t tag_size,
                           WorkQueue<PlainChunk> &encrypt_queue, FailureState &failure_state) {
            try {
                ...
                while (!failure_state.failed()) {
                    ...
                    if (!append_record_bytes_to_chunks(*record, chunk_size, current_chunk,
                                                       pending_full_chunk, next_chunk_index,
                                                       shard_writer, encrypt_queue, tag_size)) {
                        encrypt_queue.close();
                        return;
                    }
                }
                ...
                if (!failure_state.failed()) {
                    if (!current_chunk.empty()) {
                        if (pending_full_chunk) {
                            ...
                            if (!enqueue_planned_chunk(shard_writer, encrypt_queue,
                                                       next_chunk_index++, std::move(bytes), false,
                                                       tag_size)) {
                                encrypt_queue.close();
                                return;
                            }
                        ...
                        enqueue_planned_chunk(shard_writer, encrypt_queue, next_chunk_index++,
                                              std::move(current_chunk), true, tag_size);
                    } else if (pending_full_chunk) {
                        auto bytes = std::move(*pending_full_chunk);
                        pending_full_chunk.reset();

                        enqueue_planned_chunk(shard_writer, encrypt_queue, next_chunk_index++,
                                              std::move(bytes), true, tag_size);
        ...
        void encryption_worker_main(const EncryptPipelineOptions &options,
            ...
            try {
                ...
                while (!failure_state.failed()) {
                    ...
                    const auto aad = make_aad(options, job->frame_header.shard_index,
                    ...
                    const auto expected_ciphertext_and_tag_size =
                        job->frame_header.ciphertext_len +
                        static_cast<std::uint64_t>(job->frame_header.tag_len);
                    if (ciphertext.size() != expected_ciphertext_and_tag_size) {
                        throw Error("AEAD backend returned ciphertext length inconsistent with "
                                    "ChunkFrameHeaderV1");
                    }
        ...
        void ordered_writer_main(io::ShardWriter &shard_writer, WorkQueue<CipherChunk> &write_queue,
                                 FailureState &failure_state) {
                ...
                while (true) {
                    ...
                    while (true) {
                        ...
                        shard_writer.write_chunk_frame(
                            chunk.frame_header,
                            ConstByteSpan{chunk.frame_header_bytes.data(),
                                          chunk.frame_header_bytes.size()},
                            ConstByteSpan{chunk.ciphertext_and_tag.data(),
    ...
    EncryptPipeline::EncryptPipeline(EncryptPipelineOptions options,
        ...
        : options_(std::move(options)), backend_(std::move(backend)), keys_(std::move(keys)),
          archive_writer_(std::move(archive_writer)), shard_writer_(std::move(shard_writer)) {
    ...
    void EncryptPipeline::run() {
        ...
        std::thread producer([&] {
            producer_main(archive_writer_, shard_writer_, options_, tag_size, encrypt_queue,
                          failure_state);
        });
