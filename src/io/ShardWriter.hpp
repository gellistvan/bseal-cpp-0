...
namespace bseal::io {
    ...
    class ShardWriter {
      public:
        ...
        PlannedChunkFrame plan_chunk_frame(std::uint64_t chunk_index, std::uint64_t plaintext_len,
                                           std::uint64_t ciphertext_len, std::uint16_t tag_len,
                                           bool final_chunk);

        ShardWritePosition write_chunk_frame(const ChunkFrameHeaderV1 &header,
      ...
      private:
        ...
        void rewrite_shard_header(std::fstream &stream, const GlobalPublicHeaderV1 &global_header,
                                  std::uint32_t shard_index, std::uint64_t first_chunk_index,
