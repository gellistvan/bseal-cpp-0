...
namespace bseal::io {
    ...
    namespace {
        ...
        Bytes read_exact(std::istream &stream, std::size_t count, const std::filesystem::path &path,
        ...
        std::uint64_t checked_frame_body_size(const ChunkFrameHeaderV1 &frame_header) {
            const auto body_len =
                frame_header.ciphertext_len + static_cast<std::uint64_t>(frame_header.tag_len);
        ...
        ShardInfo read_shard_info(const std::filesystem::path &path) {
            ...
            const auto global_bytes = read_exact(stream, kGlobalPublicHeaderV1Size, path,
                                                 "truncated global public header");
            const auto global_header =
                parse_global_public_header(ConstByteSpan{global_bytes.data(), global_bytes.size()});

            // Read 80-byte shard header.
            const auto shard_bytes =
                read_exact(stream, kShardPublicHeaderV1Size, path, "truncated shard public header");
            const auto shard_header =
                parse_shard_public_header(ConstByteSpan{shard_bytes.data(), shard_bytes.size()});
            ...
            if (shard_header.shard_index >= global_header.shard_count) {
                throw InvalidArgument(
                    "shard_index " + std::to_string(shard_header.shard_index) + " >= shard_count " +
                    std::to_string(global_header.shard_count) + ": " + path.string());
            }
            ...
            if (file_size != expected_file_size) {
                throw InvalidArgument("file size mismatch (expected " +
                                      std::to_string(expected_file_size) + ", got " +
    ...
    std::vector<ShardInfo> ShardReader::discover(const std::filesystem::path &input_dir) {
        ...
        if (shards.empty()) {
            throw InvalidArgument("no framed .bin shard files found in input directory: " +
                                  input_dir.string());
        }

        std::sort(shards.begin(), shards.end(), [](const ShardInfo &a, const ShardInfo &b) {
    ...
    ShardReader::ShardReader(std::vector<ShardInfo> shards,
        ...
        : shards_(std::move(shards)), validation_(std::move(validation)),
          auth_key_(std::move(header_authentication_key)) {
        const auto key_span = auth_key_.as_span();
        const bool all_zero =
            std::all_of(key_span.begin(), key_span.end(), [](Byte b) { return b == Byte{0}; });
    ...
    ShardReader::ShardReader(std::vector<ShardInfo> shards, UnsafeSkipHeaderAuthenticationForTests,
                             ShardReaderValidation validation)
        : shards_(std::move(shards)), validation_(std::move(validation)), auth_key_() {
        validate_shards();
    }
    ...
    void ShardReader::validate_shards() {
        ...
        for (const auto &shard : shards_) {
            if (!shard_indices.insert(shard.shard_index()).second) {
                throw InvalidArgument("duplicate shard index: " +
                                      std::to_string(shard.shard_index()));
            }

            // Global header must be byte-for-byte identical across all shards.
            const auto this_global_bytes = serialize_global_public_header(shard.global_header);
            if (this_global_bytes != reference_global_bytes) {
                throw InvalidArgument("global header mismatch across shards: " +
                                      shard.path.string());
            }
            ...
            if (!auth_key_.empty()) {
                if (!verify_shard_header_mac(auth_key_.as_span(), shard.global_header,
                                             shard.shard_header)) {
        ...
        for (const auto &shard : shards_) {
            ...
            if (shard.first_chunk_index() != expected_first_chunk) {
                throw InvalidArgument("chunk index gap/overlap before shard " +
                                      std::to_string(shard.shard_index()));
            }
        ...
        if (expected_first_chunk != expected_total_chunk_count_) {
            throw InvalidArgument("shard set does not cover global_chunk_count");
        }
    }
    ...
    void ShardReader::open_current_shard() {
        ...
        current_stream_.seekg(static_cast<std::streamoff>(kPayloadOffset), std::ios::beg);
        if (!current_stream_) {
    ...
    void ShardReader::close_current_shard_and_check_trailing_garbage() {
        ...
        if (next != std::char_traits<char>::eof()) {
            throw InvalidArgument("trailing garbage after declared chunk frames: " +
                                  shard.path.string());
        }
    ...
    std::optional<ChunkRecord> ShardReader::read_next_chunk_record() {
        while (current_shard_pos_ < shards_.size()) {
            ...
            const auto header_bytes = read_exact(current_stream_, kChunkFrameHeaderV1Size,
                                                 shard.path, "truncated frame header");
            ...
            const auto ciphertext =
                read_exact(current_stream_, static_cast<std::size_t>(body_len_u64), shard.path,
