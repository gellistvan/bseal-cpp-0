...
namespace bseal::io {
    namespace {
        ...
        template <std::size_t N> std::array<Byte, N> read_array(Reader &reader) {
    ...
    GlobalPublicHeaderV1 parse_global_public_header(ConstByteSpan bytes) {
        ...
        if (!std::equal(h.magic.begin(), h.magic.end(), kGlobalHeaderV1Magic.begin(),
                        kGlobalHeaderV1Magic.end())) {
            throw InvalidArgument(
                "wrong global header magic (old BSEAL01\\0 format is not supported)");
        }
        ...
        if (!is_power_of_two(h.chunk_plain_size) || h.chunk_plain_size < 65536u ||
        ...
        case 1: // chunk — padded_plaintext_size must be a positive multiple of chunk_plain_size
            ...
            if (h.padded_plaintext_size % h.chunk_plain_size != 0) {
                throw InvalidArgument("padded_plaintext_size is not a multiple of chunk_plain_size "
                                      "for 'chunk' padding policy");
            }
            if (h.final_plaintext_chunk_len != h.chunk_plain_size) {
                throw InvalidArgument("final_plaintext_chunk_len must equal chunk_plain_size "
                                      "for 'chunk' padding policy");
            }
            break;
        case 2: { // power2 — padded_plaintext_size must be a power of two and a multiple of
                  // chunk_plain_size
            if (h.padding_policy_value != 0) {
                throw InvalidArgument("padding_policy_value must be 0 for 'power2' padding policy");
            }
            if (!is_power_of_two(h.padded_plaintext_size)) {
                throw InvalidArgument("padded_plaintext_size is not a power of two "
        ...
        case 3: // fixed-size — padding_policy_value must equal padded_plaintext_size and be a chunk
                // multiple
            if (h.padding_policy_value != h.padded_plaintext_size) {
                throw InvalidArgument("padding_policy_value must equal padded_plaintext_size "
            ...
            break;
        default:
            throw InvalidArgument("unsupported padding_policy_id: " +
                                  std::to_string(h.padding_policy_id));
        }
    ...
    ShardPublicHeaderV1 parse_shard_public_header(ConstByteSpan bytes) {
        ...
        if (!std::equal(h.shard_magic.begin(), h.shard_magic.end(), kShardHeaderV1Magic.begin(),
                        kShardHeaderV1Magic.end())) {
    ...
    std::array<Byte, 32> compute_public_header_hash(const GlobalPublicHeaderV1 &global_header,
        ...
        blake3_hasher_update(&hasher, kPublicHeaderHashDomain.data(),
                             kPublicHeaderHashDomain.size());
    ...
    std::array<Byte, 32> compute_shard_header_mac(ConstByteSpan header_authentication_key,
        ...
        message.insert(message.end(), reinterpret_cast<const Byte *>(kHeaderMacDomain.data()),
        ...
        auto *result =
            HMAC(EVP_sha256(), header_authentication_key.data(),
                 checked_int_size(header_authentication_key.size(), "header authentication key"),
                 message.data(), message.size(), out.data(), &out_len);
    ...
    bool verify_shard_header_mac(ConstByteSpan header_authentication_key,
                                 const GlobalPublicHeaderV1 &global_header,
                                 const ShardPublicHeaderV1 &shard_header) {
        const auto expected =
            compute_shard_header_mac(header_authentication_key, global_header, shard_header);

        return CRYPTO_memcmp(expected.data(), shard_header.header_mac.data(), expected.size()) == 0;
    }
    ...
    namespace {
        ...
        void validate_chunk_frame_header_v1(const ChunkFrameHeaderV1 &header) {
            ...
            if (header.tag_len >
                static_cast<std::uint16_t>(std::numeric_limits<std::size_t>::max())) {
    ...
    ChunkFrameHeaderV1 parse_chunk_frame_header_v1(ConstByteSpan bytes) {
        ...
        if (!std::equal(magic.begin(), magic.end(), kChunkFrameV1Magic.begin(),
                        kChunkFrameV1Magic.end())) {
        ...
        // FORMAT.md rejection: tag_len must be 16, ciphertext_len must equal plaintext_len (v1
        // AEADs).
        ...
        if (header.ciphertext_len != static_cast<std::uint64_t>(header.plaintext_len)) {
            throw InvalidArgument(
                "chunk frame ciphertext_len must equal plaintext_len for v1 AEADs");
        }
    ...
    std::uint64_t chunk_frame_v1_encoded_size_from_params(std::uint64_t plaintext_len,
