...
namespace bseal::io {
    ...
    ShardWriter::ShardWriter(ShardWriterOptions options) : options_(std::move(options)) {
    ...
    void ShardWriter::validate_shard_hash_vector() {
        ...
        if (hashes.size() != static_cast<std::size_t>(options_.global_header.shard_count)) {
            throw InvalidArgument("ShardWriter per_shard_public_header_hashes size does not match "
    ...
    void ShardWriter::rewrite_shard_header(std::fstream &stream,
                                           const GlobalPublicHeaderV1 &global_header,
                                           std::uint32_t shard_index,
                                           std::uint64_t first_chunk_index,
                                           std::uint64_t chunk_count, std::uint64_t payload_len) {
        ...
        sh.header_mac = compute_shard_header_mac(options_.header_authentication_key.as_span(),
                                                 global_header, sh);
    ...
    void ShardWriter::close_current_shard() {
...

        {
            ...
            rewrite_shard_header(rw, options_.global_header, current_shard_index_,
                                 current_first_chunk_index_, current_chunk_count_,
    ...
    void ShardWriter::write_raw(ConstByteSpan bytes) {
        ...
        current_stream_.write(reinterpret_cast<const char *>(bytes.data()),
    ...
    ShardWritePosition ShardWriter::write_chunk_frame(const ChunkFrameHeaderV1 &header,
        ...
        if (header_bytes.size() != expected_header_bytes.size() ||
            !std::equal(header_bytes.begin(), header_bytes.end(), expected_header_bytes.begin(),
        ...
        if (frame_size > options_.max_shard_payload_len) {
            throw InvalidArgument("chunk frame (" + std::to_string(frame_size) +
                                  " bytes) exceeds"
                                  " max_shard_payload_len (" +
                                  std::to_string(options_.max_shard_payload_len) +
    ...
    void ShardWriter::finish() {
        ...
        if (total_chunks > 0 && final_chunk_plaintext_len_ > 0) {
            ...
            final_global.padded_plaintext_size = checked_add_u64(
                ...
                final_chunk_plaintext_len_, "ShardWriter: padded plaintext size");
        }
        ...
        for (const auto &fs : finalized_shards_) {
            ...
            rw.write(reinterpret_cast<const char *>(global_bytes.data()),
            ...
            rewrite_shard_header(rw, final_global, fs.shard_index, fs.first_chunk_index,
                                 fs.chunk_count, fs.payload_len);
        }

        finished_ = true;
    }

    PlannedChunkFrame ShardWriter::plan_chunk_frame(std::uint64_t chunk_index,
                                                    std::uint64_t plaintext_len,
                                                    std::uint64_t ciphertext_len,
                                                    std::uint16_t tag_len, bool final_chunk) {
        if (finished_) {
        ...
        if (chunk_index != planned_next_chunk_index_) {
            throw InvalidArgument(
                "ShardWriter frame planner requires contiguous ascending chunk indexes");
        }
