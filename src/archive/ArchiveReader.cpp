...
namespace bseal::archive {
    ...
    ArchiveReader::ArchiveReader(ArchiveReaderOptions options) : options_(std::move(options)) {
        ...
        if (use_hardened_tmp) {
            ...
            const int root_fd = ::open(options_.output_root.c_str(),
                                       O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW);
            if (root_fd == -1) {
                const int saved = errno;
                throw InvalidArgument("cannot open output root for hardened temp creation: " +
                                      std::string(std::strerror(saved)));
            }
            if (::mkdirat(root_fd, temp_name.c_str(), 0700) == -1) {
                const int saved = errno;
                ::close(root_fd);
                throw InvalidArgument("cannot create temporary extraction directory '" + temp_name +
                                      "': " + std::strerror(saved));
            }
            ::close(root_fd);
        } else {
            ...
            if (ec) {
                throw InvalidArgument("cannot create temporary extraction directory: " +
                                      ec.message());
            }
#if !defined(_WIN32)
        }
    ...
    ArchiveReader::ArchiveReader(ArchiveReader &&other) noexcept
        : options_(std::move(other.options_)), temp_root_(std::move(other.temp_root_)),
    ...
    void ArchiveReader::create_directory(const EntryMetadata &metadata) {
        ...
        // Apply directory timestamps after all children are restored; creating children updates
        // mtime.
    ...
    void ArchiveReader::begin_file(const EntryMetadata &metadata) {
        ...
        if (ec) {
            throw InvalidArgument("cannot create parent directory for extracted file: " +
                                  ec.message());
        }
    ...
    void ArchiveReader::commit_temp_tree() {
        SafeOutputTree safe_tree(options_.output_root, options_.hardened_extract_mode);
    ...
    void ArchiveReader::apply_metadata_to_path(const std::filesystem::path &path,
        ...
        if (options_.restore_permissions && metadata.posix_mode != 0) {
            std::filesystem::permissions(path, bits_to_permissions(metadata.posix_mode),
        ...
        if (options_.restore_timestamps && metadata.times.modified_ns_since_unix_epoch != 0 &&
            ...
            std::filesystem::last_write_time(
                path, unix_ns_to_file_time(metadata.times.modified_ns_since_unix_epoch), ec);
    ...
    std::filesystem::path
    ArchiveReader::temp_path_for(const std::filesystem::path &archive_path) const {
        return make_safe_output_path(temp_root_, archive_path);
    }

    std::filesystem::path
    ArchiveReader::final_path_for(const std::filesystem::path &archive_path) const {
