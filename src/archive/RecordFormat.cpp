...
namespace bseal::archive {
    namespace {
        ...
        void append_string(Bytes &out, const std::string &value) {
            ...
            append_bytes(out,
                         ConstByteSpan{reinterpret_cast<const Byte *>(value.data()), value.size()});
        }
        ...
        std::string path_to_archive_string(const std::filesystem::path &path) {
            ...
            // The archive format treats paths as UTF-8 by convention. A production Windows build
            // may want to replace this with explicit UTF-8 conversion from native wide paths.
    ...
    ArchiveRecord parse_record(ConstByteSpan bytes) {
        ...
        return ArchiveRecord{static_cast<RecordType>(type_byte),
