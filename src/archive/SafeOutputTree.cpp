...
namespace bseal::archive {
    ...
    SafeOutputTree::SafeOutputTree(const std::filesystem::path &root, HardenedExtractMode mode)
        : root_path_(root) {
        if (mode == HardenedExtractMode::On && !is_platform_supported()) {
            throw InvalidArgument("--hardened-extract=on is not supported on this platform; "
                                  "use --hardened-extract=auto or --hardened-extract=off");
        }

        const bool want_hardened = (mode == HardenedExtractMode::On) ||
        ...
        if (want_hardened) {
            ...
            if (root_fd_ == -1) {
                ...
                throw InvalidArgument("hardened extraction: cannot open output root '" +
                                      root.string() + "': " + std::strerror(saved));
            }
            hardened_ = true;
        }
#else
    ...
    int SafeOutputTree::open_dir_hardened(const std::filesystem::path &rel_dir_path) {
        ...
        if (current.fd == -1) {
            throw InvalidArgument("hardened extraction: dup(root_fd) failed: " +
        ...
        for (const auto &component : rel_dir_path) {
            ...
            if (name == "..") {
                ...
                throw InvalidArgument("hardened extraction: '..' path component rejected");
            }

            // Create the directory if it does not exist yet; EEXIST is fine.
            if (::mkdirat(current.fd, name.c_str(), 0755) == -1 && errno != EEXIST) {
                const int saved = errno;
                throw InvalidArgument("hardened extraction: mkdirat('" + name +
                                      "'): " + std::strerror(saved));
            }
            ...
            if (::fstatat(current.fd, name.c_str(), &st, AT_SYMLINK_NOFOLLOW) == -1) {
                const int saved = errno;
                throw InvalidArgument("hardened extraction: fstatat('" + name +
                                      "'): " + std::strerror(saved));
            }

            if (S_ISLNK(st.st_mode)) {
                throw InvalidArgument("hardened extraction: symlink at path component '" + name +
                                      "'; aborting to prevent path escape outside output root");
            }

            if (!S_ISDIR(st.st_mode)) {
                throw InvalidArgument("hardened extraction: expected directory at '" + name +
            ...
            if (next.fd == -1) {
                const int saved = errno;
                throw InvalidArgument("hardened extraction: openat('" + name +
                                      "'): " + std::strerror(saved));
            }
    ...
    void SafeOutputTree::rename_into(const std::filesystem::path &src_abs,
        ...
        if (hardened_) {
            ...
            struct ::stat st{};
            const bool exists =
                (::fstatat(parent_fd.fd, dest_name.c_str(), &st, AT_SYMLINK_NOFOLLOW) == 0);

            if (exists) {
                if (!overwrite) {
                    throw InvalidArgument("output path already exists: " +
                ...
                if (::unlinkat(parent_fd.fd, dest_name.c_str(), 0) == -1) {
                    const int saved = errno;
                    throw InvalidArgument("hardened extraction: unlinkat('" + dest_name +
                                          "'): " + std::strerror(saved));
                }
            }
            ...
            if (::renameat(AT_FDCWD, src_abs.c_str(), parent_fd.fd, dest_name.c_str()) == -1) {
                const int saved = errno;
                throw InvalidArgument("hardened extraction: renameat('" + dest_name +
                                      "'): " + std::strerror(saved));
            }
            return;
        }
...

        {
            ...
            if (dst_present) {
                if (!overwrite) {
                    throw InvalidArgument("output path already exists: " + dest_abs.string());
                }
                ...
                if (rm_ec) {
                    throw InvalidArgument("cannot remove existing output path: " + rm_ec.message());
                }
            }
        }

        {
            ...
            if (ec) {
                throw InvalidArgument("cannot create parent directory: " + ec.message());
            }
        }
        ...
        // Guard against a pre-existing symlink redirecting the write outside root.
        {
            ...
            const auto real_parent = std::filesystem::canonical(dest_abs.parent_path(), canon_ec);
            if (!canon_ec) {
                const auto rel = real_parent.lexically_relative(root_path_);
                if (!rel.empty() && *rel.begin() == "..") {
                    throw InvalidArgument("output path escapes output root via symlink: " +
                                          dest_abs.string());
                }
            }
        }
