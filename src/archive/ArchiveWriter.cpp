...
namespace bseal::archive {
    ...
    ArchiveWriter::ArchiveWriter(ArchiveWriterOptions options) : options_(std::move(options)) {
        ...
        for (std::filesystem::recursive_directory_iterator
                 it(options_.input_root, iterator_options, ec),
             end;
        ...
        // Deterministic archive stream order. The outer encryption layer hides these names later,
        // but deterministic ordering is useful for tests and reproducible pre-encryption behavior.
        std::ranges::sort(entries_, [this](const auto &a, const auto &b) {
            const auto ar =
                std::filesystem::relative(a.path(), options_.input_root).generic_string();
            const auto br =
                std::filesystem::relative(b.path(), options_.input_root).generic_string();
            return ar < br;
        });
    }
    ...
    std::optional<Bytes> ArchiveWriter::next_file_bytes_or_end() {
        ...
        if (current_file_bytes_read_ != current_file_expected_bytes_) {
            throw InvalidArgument("file changed size during encryption: '" +
                                  ...
                                  std::to_string(current_file_expected_bytes_) +
                                  " bytes but read " + std::to_string(current_file_bytes_read_));
        }
